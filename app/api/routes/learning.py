...
from app.api.dependencies import get_authorization_service, get_current_user, get_learning_service, rate_limit_api_read, rate_limit_api_write
...
@router.post("/learning-plans", response_model=LearningPlanResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(rate_limit_api_write)])
...
@router.get("/learning-plans/{plan_id}", response_model=LearningPlanResponse, dependencies=[Depends(rate_limit_api_read)])
...
@router.get("/students/{student_id}/active-learning-plan", response_model=LearningPlanResponse, dependencies=[Depends(rate_limit_api_read)])
...
@router.patch("/learning-activities/{activity_id}", response_model=LearningActivityResponse, dependencies=[Depends(rate_limit_api_write)])
