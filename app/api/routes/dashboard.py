...
from app.api.dependencies import get_audit_service, get_authorization_service, get_current_user, get_dashboard_service, rate_limit_api_admin, rate_limit_api_read, rate_limit_api_write, require_admin
...
@router.post('/access-grants',response_model=AccessGrantResponse,status_code=status.HTTP_201_CREATED,dependencies=[Depends(rate_limit_api_admin)])
...
@router.get('/students/{student_id}',response_model=StudentDashboardResponse,dependencies=[Depends(rate_limit_api_read)])
...
@router.get('/viewers/{viewer_id}/overview',response_model=ViewerOverviewResponse,dependencies=[Depends(rate_limit_api_read)])
...
@router.post('/students/{student_id}/snapshots',response_model=SnapshotResponse,status_code=status.HTTP_201_CREATED,dependencies=[Depends(rate_limit_api_write)])
...
@router.get('/students/{student_id}/trends',response_model=TrendResponse,dependencies=[Depends(rate_limit_api_read)])
